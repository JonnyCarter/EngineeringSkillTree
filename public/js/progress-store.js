(function () {
  ...
  function normalize(tree, data = {}) {
    ...
    for (const skillId of data.completedSkills || []) {
      if (validIds.has(skillId) && !records[skillId]) {
        records[skillId] = { status: 'self_assessed', confidence: 2, updatedAt: null };
      }
    }
    return records;
  }
  ...
  function careerLevel(items) {
    const earned = items.filter((item) => item.earned);
    if (earned.filter((item) => Number(item.tier) === 4).length >= 3) return { id: 'staff', name: 'Staff', next: null };
    if (earned.filter((item) => Number(item.tier) === 3).length >= 3) return { id: 'senior', name: 'Senior', next: 'Staff' };
  ...
  function snapshot(tree, records) {
    ...
    const nodes = tree.nodes.map((node) => {
      ...
      return {
        ...
        confidenceLabel: confidenceLabels[record?.confidence] || null, xp
      };
    });
    ...
    return {
      ...
      selfAssessedCount: completed.size,
      totalXp: nodes.reduce((sum, node) => sum + node.xp, 0), careerLevel: careerLevel(milestoneProgress),
